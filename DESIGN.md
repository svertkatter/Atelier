...
- **URL/DOIからのインポート**（PDFなしで登録可能）：識別子を入力してメタデータを取得・確認し、
  PDFを添付せず `library/{key}/meta.md` のみで登録する。CSL-JSON上のidが安全なキー
  （`^[A-Za-z][A-Za-z0-9_-]*$`）でない場合（DOI・CiNii CRID等）は必ず`CitationKey.generate`
  でフォルダ名を再生成する（DOIの`/`をフォルダ階層に持ち込まない。過去バグの再発防止）。
  PDFは後からリーダー画面の「PDFを添付」ボタンで追加できる
- タグ管理、著者・年・キーワード検索
- 各行のメニュー（PopupMenu）：メタデータ編集／プロジェクトへ追加／削除
  - 削除：参照しているプロジェクト一覧を確認ダイアログで表示 → 確定後
    `library/{key}/` を削除し、全project.jsonのrefsから当該keyを除去（`LibraryDelete`）
  - プロジェクトへ追加：既存プロジェクト一覧から選択、またはその場で新規作成して追加
    （`ProjectStore.ensureRef`）
- 行タップ＝論文を選択し、**リーダーへ自動遷移**する（モード選択はRiverpodの
  `selectedModeProvider`に昇格。ライブラリ／リーダー間の行き来を減らすため）
...
- **左に折りたたみ可能な論文一覧サイドバー**（タイトル・著者・年、現在開いている論文をハイライト、
  クリックで切替）。PDFのない論文（URL/DOIインポート由来）は「PDFなし」と表示される
- PDFのない論文を選択した場合はビューアの代わりに「PDFを添付」ボタンを表示し、
  file_pickerで選んだPDFを`library/{key}/paper.pdf`にコピーして即座に表示する
